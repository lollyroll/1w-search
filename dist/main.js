...
/*!
 ...
 Changes made by Eugene Lobanov 12.10.2015 <l_eugene@inbox.ru>
 diff https://bitbucket.org/1worldonline/1w-main/commits/c38dba0d79d31e4a2d1eeb3e41d0d51d4b92d86a
 
 Extended @member Backgrid.HeaderCell onClick with 

    reversed sort order 'descending', 'ascending', 'default'
    cycleReverseSort(header, col)
    @param {Object, Object} header, col

    sort 'ascending', 'descending', 'default' is applyed for subcolumns
    defined in sortingSubColumns @param{null|array}

    cycleSubColumnSort(header, col) header, col
    @param {Object, Object}

    sort 'descending', 'ascending', 'default' is applyed for subcolumns
    defined in sortingSubColumns @param{null|array}

    cycleReverseMultiColumnSort(header, col) header, col
    @param {Object, Object}

    cycle through sortingSubColumns array and each time returns currentSortColumn name {string}
    cycleNextSortingColumn()
    @return {string}
 */
...
(function (root, factory) {
    ...
    if (typeof define === "function" && define.amd) {
        ...
        define('backgrid',["underscore", "backbone"], function (_, Backbone) {
...
// http://es5.github.com/#x15.5.4.20
    ...
    var Column = Backgrid.Column = Backbone.Model.extend({
        ...
        /**
         ...
         @param {"toggle"|"cycle"|"cycleReverseSort"|"cycleSubColumnSort"|"cycleReverseMultiColumnSort"}  [attrs.sortType="cycle"]
    ...
    var HeaderCell = Backgrid.HeaderCell = Backbone.View.extend({
        ...
        /**
         ...
         `ascending`, `descending`, and default. If 'sortType' defined as 'cycleReverseMultiColumnSort'
         or 'cycleReverseMultiColumnSort' then sorting cycling will be applyed for each subcolumn defined in
         'sortingSubColumns' @param {null|array} sortingSubColumns and will be cycled through them.
         Takes array of formated strings which are names of sub columns(columns).
         If defined currentSortColumn  @param {null|string} currentSortColumn then cycle starts from currentSortColumn in round.
         currentSortColumn will be applyed as class name for header cell.
         */
        onClick: function (e) {
            e.preventDefault();
            ...
            var currentDirection = column.get("direction");
            var subColumnsArray = column.get("sortingSubColumns");
            var currentSortColumn = column.get("currentSortColumn");
            var self = this;

            function cycleSort(header, col) {
                if (currentDirection === "ascending") collection.trigger(event, col, "descending");
                else if (currentDirection === "descending") collection.trigger(event, col, null);
            ...
            function cycleReverseSort(header, col) {
                if (column.get("direction") === "descending") collection.trigger(event, col, "ascending");
                else if (column.get("direction") === "ascending") collection.trigger(event, col, null);
                else collection.trigger(event, col, "descending");
            }

            function cycleSubColumnSort(header, col) {
                if (currentDirection === "ascending") collection.trigger(event, col, "descending", currentSortColumn);
                else if (currentDirection === "descending") collection.trigger(event, col, null, currentSortColumn);
                else collection.trigger(event, col, "ascending", cycleNextSortingColumn());
            }

            function cycleReverseMultiColumnSort(header, col) {
                if (column.get("direction") === "descending") collection.trigger(event, col, "ascending", currentSortColumn);
                else if (column.get("direction") === "ascending") collection.trigger(event, col, null, currentSortColumn);
                else collection.trigger(event, col, "descending", cycleNextSortingColumn());
            }

            function cycleNextSortingColumn() {
                var customSortValueLength,
                    currentPosInSortingKeysArray,
                    curentValue = currentSortColumn;

                if (subColumnsArray) {
                    customSortValueLength = subColumnsArray.length;
                    currentPosInSortingKeysArray = _.indexOf(subColumnsArray, curentValue);

                    if (currentPosInSortingKeysArray != -1 && currentPosInSortingKeysArray != customSortValueLength - 1) {
                        self.$el.removeClass(curentValue);
                        curentValue = subColumnsArray[currentPosInSortingKeysArray + 1];
                        self.$el.addClass(curentValue);
                    }
                    else {
                        self.$el.removeClass(curentValue);
                        curentValue = subColumnsArray[0];
                        self.$el.addClass(curentValue);
                    }
                    column.set("currentSortColumn", curentValue);
                }
            ...
            if (sortable) {
                ...
                if (sortType === "toggle") {
                    ...
                    if (sortType === "cycleReverseSort") {
                        cycleReverseSort(this, column);
                    }
                    else if (sortType === "cycleSubColumnSort") {
                        cycleSubColumnSort(this, column);
                    }
    ...
    var Body = Backgrid.Body = Backbone.View.extend({
        ...
        /**
         ...
         @param {null|string} subColumnName

         See [Backbone.Collection#comparator](http://backbonejs.org/#Collection-comparator)
         */
        sort: function (column, direction, subColumnName) {
            if (!_.contains(["ascending", "descending", null], direction)) {
                throw new RangeError('direction must be one of "ascending", "descending" or `null`');
            }

            if (_.isString(column)) column = this.columns.findWhere({name: column});
            var columnName = subColumnName || column.get("name");
            var collection = this.collection;
            ...
            var comparator = this.makeComparator(columnName, order,
                order ?
                    column.sortValue() :
                    function (model) {
            ...
            if (Backbone.PageableCollection &&
                ...
                collection.setSorting(order && columnName, order,
